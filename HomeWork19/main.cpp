...
void main()
{
	...
	/*char str_ru[] = "АБВГДЕЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдежзийклмнопрстуфхцчшщъыьэюяЁё";
	...
	char str_4[] = "Лёша на полке клопа нашёл";
...
{	
	...
	for (; str[i]; ++i)
	{
	if (str[i]>=97 && str[i]<=122) str[i] -= 32; //маленькие латинские (91 - 96 не буквы)
	if (str[i]>=-32 && str[i]<=-1) str[i] -= 32; //маленькие кириллица (от 'а' до 'я' : от -32 до -1)
	if (str[i] ==-72) str[i] = -88; // буквы ё(72) и Ё(88)
	}
}
...
void to_lower(char str[])
{
	...
	if (str[i] >= -64 && str[i] <= -33)	str[i] += 32; //большие кириллица (от 'А' до 'Я' : от -64 до -33)
...
{	
	...
	{			
		if (str[i] == ' ') ++i;
		if (str[end] == ' ') --end;
		//if (str[i] >= 65 && str[i] <= 90) str[i] += 32; //Модифицирует строку
		//if (str[i] >= -64 && str[i] <= -33)	str[i] += 32; //Модифицирует строку
